...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 25 19:29:12 2025
...
os.chdir('/Users/zoeatoy/Downloads/capstone/')
...
import argparse
import os
import sys  # Add this since you use sys.path.append below

sys.path.append('/Users/zoeatoy/Downloads/capstone/')
...
def main(args):
    ...
    D_NLLS, D_star_NLLS, F_NLLS = ivim_fit(test)  # Unpack all three diffusion parameters
    end = time.time()
    ...
    # calculate_stats(
    #     (D_test, F_test),
    #     (D_pia_stacked, F_pred.detach().cpu().numpy()),
    #     (D_NLLS_stacked, F_NLLS)
    # )
    
    calculate_stats(
    (D_test.detach().cpu().numpy() if torch.is_tensor(D_test) else D_test,
     F_test.detach().cpu().numpy() if torch.is_tensor(F_test) else F_test),
    (D_pia_stacked, F_pred.detach().cpu().numpy()),
    (D_NLLS_stacked, F_NLLS)
)
    ...
    start = time.time()
    D_hybrid, D_star_hybrid, F_hybrid = ivim_fit(test)
    end = time.time()
